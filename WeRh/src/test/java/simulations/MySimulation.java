...
public class MySimulation extends Simulation {
    ...
    /**
     * Renews the token shared by all the virtual users. Not injected below because
     * a short run
     * fits in a single token lifetime — for a run longer than that, add it to setUp
     * next to the
     ...
     */
    ScenarioBuilder tokenRefresher = scenario("Token refresh").exec(LoginGroup.keepTokenFresh(Duration.ofMinutes(30)));

    /**
     * First connection of a user: the GCU approval dialog shows up once and has to
     * be
     * accepted. Kept for reference but not injected below — to test it, swap the
     * scenario passed to setUp.
     */
    ScenarioBuilder firstConnection = scenario("WeRH first connection").exec(
    ...
    ScenarioBuilder userJourney = scenario("WeRH user journey").exec(
            ApiHeaders.initTenants,
            WebPages.home,
            pause(1),
            LoginGroup.login,
    ...
    ScenarioBuilder agentsList = scenario("WeRH agents list").exec(
            ApiHeaders.initTenants,
            WebPages.home,
            pause(1),
            LoginGroup.login,
    ...
    /**
     * Returning user opening an agent from the list, then its pay-stub (bulletin)
     * tab.
     */
    ScenarioBuilder agentBulletin = scenario("WeRH agent bulletin").exec(
            ApiHeaders.initTenants,
            WebPages.home,
            pause(1),
            LoginGroup.login,
    ...
    /**
     * Returning user opening the pay assistant, controlling the bulletins, then
     * recomputing them.
     * The recompute step opens the SSE stream that reports the calculation
     * progress.
     */
    ScenarioBuilder payControlBulletins = scenario("WeRH pay control bulletins").exec(
            ApiHeaders.initTenants,
            WebPages.home,
            pause(1),
            LoginGroup.login,
    ...
    ScenarioBuilder ouvertureMoisPaie = scenario("Ouverture mois paie").exec(
            ApiHeaders.initTenants,
            WebPages.home,
            pause(1),
            LoginGroup.login,
    ...
    ScenarioBuilder payVisualiserBulletinsEtatCaisseBordereau = scenario("WeRH pay visualiser bulletins").exec(
            ApiHeaders.initTenants,
            WebPages.home,
            pause(1),
            LoginGroup.login,
    ...
    ScenarioBuilder payVisualiserBulletinsEtatCaisse = scenario("WeRH pay visualiser bulletins").exec(
            ApiHeaders.initTenants,
            WebPages.home,
            pause(1),
            LoginGroup.login,
